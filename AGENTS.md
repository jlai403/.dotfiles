...
The `omarchy-shell` package also ships `~/.local/bin/omarchy-bar-stats` (stowed from `.local/bin/omarchy-bar-stats`), the script behind the bar's custom **`stats` command module** (`{ "id": "stats", "type": "command", "exec": "~/.local/bin/omarchy-bar-stats", "interval": 3 }` in `shell.json`'s center layout, right of `omarchy.weather`). The bar renders command modules via Bar.qml's `CustomCommandModule` (a `WidgetButton`): stdout's **last line** is parsed as Waybar-style JSON (`text`/`tooltip`/`class` — plain text falls back to `text`); `class: "active"` tints the label to `Color.urgent` via `WidgetButton.active`/`useActiveColor`. The script prints CPU% (from a 0.4s `/proc/stat` delta), RAM used/total (`/proc/meminfo`), root disk % (`df`), per-second NET rx/tx (`/proc/net/dev` delta, minus `lo`), and package temp (`hwmon` `coretemp`/`k10temp`/`cpu_thermal`), alerting above 90% CPU / 90% RAM / 95°C. Pitfalls baked in: `set -e` + `read … < <(…)` process-substitution abort, and `$(<file)` failing on missing files while still printing to stderr — both avoided with command substitution / `cat … || continue`. |
