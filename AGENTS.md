...
| `hypr` | `~/.config/hypr/` | Hyprland **user overrides** (`bindings.lua`, `input.lua`, `monitors.lua`, `looknfeel.lua`) + `~/.local/bin/omarchy-hyprland-window-quit-app`. `bindings.lua` is the mac-keyboard parity layer: Cmd shims forward SUPER chords as CTRL equivalents via `mac_shortcut` (silent in terminals); workspace nav on ALT+digits, swap on ALT+SHIFT+hjkl; launcher re-homes live on SUPER+CTRL(+SHIFT) — releasing a chord there requires also unbinding omarchy's hidden `SUPER+CTRL` toggles (utilities.lua); window state on HYPER (CapsLock chord, `keyd` pkg). `looknfeel.lua` carries the look deltas only (gaps_in 10 = aerospace parity, border_size 1). The entry point and remaining templates (`hyprland.lua`, `autostart.lua`, `hyprsunset.conf`, `xdph.conf`) are omarchy-owned real files on disk — NOT tracked; `hyprland.lua` requires `hypr.looknfeel`/`hypr.autostart`, so those two must exist (`looknfeel.lua` is stowed over omarchy's template slot; adopting a template file = rm the real file first, then re-stow) |
