...
- `omarchy/plugins.yml` — Omarchy shell plugin manifest consumed by `omarchy/setup.zsh`'s `_install_omarchy_plugins`; each entry is a git clone into `~/.config/omarchy/plugins/<id>/` (real checkout, not stowed), with `enable: true` passing `--enable`; skipped when the plugin dir already exists
