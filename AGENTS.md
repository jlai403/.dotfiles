...
| `stats` | `~/.config/omarchy/plugins/jlai.stats/` + `~/.local/bin/omarchy-bar-stats` | A **third-party bar-widget plugin** (id `jlai.stats`, authored — not a clone) replacing the old `stats` command module: CPU / RAM / temp in the bar show `icon · value · micro-sparkline`; disk shows `icon · usage% · dual read/write activity bars` (read top / write bottom, one shared scale). Nerd glyphs: CPU `fa-microchip U+F2DB`, RAM `mdi-memory U+F035B`, disk `mdi-harddisk U+F02CA`, temp `mdi-thermometer U+F050F` (note `mdi-temperature-celsius U+F0504` renders as a bare "C" — don't use it). Each metric is tinted `bar.urgent` independently above its own threshold (CPU/RAM > 80%, disk > 90%, temp > 90 °C). Left-click opens a `KeyboardPanel` popup (iStat-style, Flickable) built from `Panel`/`PanelSectionHeader`/`PanelKeyCatcher` with `Sparkline.qml` (Canvas area/bars) graphs: CPU (hero + graph + load avg + uptime + per-core meters + top CPU procs), Memory (hero + graph + used/total + swap + cached + top RAM procs), Disk (hero + meter + used/free/mount + I/O-throughput graph + peak), Temperature (hero + graph + min/max). `Sampler.qml` primes a 300-sample in-memory ring from the script's cache once (`--history`), then appends each 2s poll; top procs refresh every 3s while open (`--top`). The script `omarchy/stats/.local/bin/omarchy-bar-stats` (stowed to `~/.local/bin`) does the `/proc` math + diskstats I/O deltas and persists `~/.cache/omarchy-bar-stats/history.ndjson` (~10 min @ 2s, trimmed to 300); modes: default sample+append, `--history N`, `--top`. Same bash pitfalls as before (`set -e` + `read … < <(…)`, missing-file `$(<file)`). Enabled by the `{ "id": "jlai.stats" }` entry in `shell.json`'s center layout, right of `omarchy.weather`. Stowed with `_stow_group omarchy stats` |
