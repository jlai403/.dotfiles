...
Item {
  ...
  property string lastPointerPosition: ""
  ...
  function dismissScreensaver(reason) {
    if (!root.idledThisCycle || root.screensaverWindowCount === 0) return
    logEvent("screensaver-dismiss", reason || "requested")
    // SIGTERM to the screensaver window lets omarchy-screensaver restore the
    // cursor and exit; the window closing then cancels the idle cycle through
    // the existing closewindow handler.
    runProcess(screensaverDismissProcess, "screensaver-dismiss",
      "pkill -f '[o]rg.omarchy.screensaver' || hyprctl dispatch closewindow class:org.omarchy.screensaver")
  }
  ...
  // Dismiss the screensaver on pointer motion. Quickshell's IdleMonitor only
  // reports idle<->active transitions, and the launch itself already consumed
  // the active edge, so poll the cursor position while the screensaver is up.
  Timer {
    id: pointerWatchTimer
    interval: 1000
    repeat: true
    running: root.idleEnabled && root.idledThisCycle && root.screensaverStartedThisCycle && root.screensaverWindowCount > 0
    onTriggered: pointerProbe.running = true
  }
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(line) {
        var position = String(line).trim()
        if (!position) return
        if (root.lastPointerPosition === "") {
          root.lastPointerPosition = position
        } else if (position !== root.lastPointerPosition) {
          root.lastPointerPosition = position
          root.dismissScreensaver("pointer-moved")
        }
      }
    }
  }
  ...
  Process {
    ...
    onExited: function(exitCode, exitStatus) { root.logEvent("process-exit", "screensaver-dismiss exitCode=" + exitCode + " status=" + exitStatus) }
  }
  Process {
    id: lockProcess
